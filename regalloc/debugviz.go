...
func maptorecord(fn *ir.Func, l map[ir.ID]struct{}) string {
	ret := ""
	for v := range l {
		val := v.ValueIn(fn)
...
func maptorecordsrc(fn *ir.Func, prefix string, l map[ir.ID]struct{}, src map[ir.ID]string) string {
	ret := ""
	for v := range l {
		val := v.ValueIn(fn)
		ret += fmt.Sprintf("<td port=\"%s\">%s:%s</td>", val.IDString(), val.IDString(), val.Reg().String())
